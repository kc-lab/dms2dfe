...
if (test_dataset=='1') or ("TEM1" in test_dataset):
    ...
    fn="Data_S1-S4.xlsx"
    data_input_dh="TEM1_Firnberg_et_al_2014/data_input"
    ...
    data=pd.read_excel("%s/data_input/%s" % (prj_dh,fn),"S1 Codon fitnesses")
    ...
    if data_type=='1':
        ...
        #     break
            ...
            for i in range(len(data_aacounts)):
                ...
                if data_aacounts.loc[i,"mut"]==data_aacounts.loc[i,"ref"]:
                    data_aacounts.loc[i,"NiScutlog"]=data_aacounts.loc[i,"NiAcutlog"]    
            data_aacounts.to_csv("%s/data_lbl/aas/%s" % (prj_dh,splitext(splitext(basename(fh))[0])[0]))
        ...
        logging.error("[valid inputs: 1 | 2]")
else:
    logging.error("[valid inputs: 1 | 2]")
